...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean deliteTurn = false;
...
//===============================================================================================
                ...
                builder.setTitle("Введите никнемус заметкуса");
...
//Заканчиваем вертилово
            ...
            {
                if (!deliteTurn){
                    Toast.makeText(getApplicationContext(),"Кликните на то чо хотите удалить",Toast.LENGTH_SHORT).show();
                    fruits.set(1,"Удалить ON");
                    adapter1.notifyDataSetChanged();
                    deliteTurn=!deliteTurn;
                } else if (deliteTurn) {
                    Toast.makeText(getApplicationContext(),"Режим удаления выключен",Toast.LENGTH_SHORT).show();
                    fruits.set(1,"Удалить Off");
                    adapter1.notifyDataSetChanged();
                    deliteTurn=!deliteTurn;
                }
...
//                    }
            ...
            {
                if ((position!=1)&(position!=0)&!deliteTurn){
                Intent intent = new Intent(MainActivity.this, ListZamet.class);
                intent.putExtra("idpoz", position);
                startActivity(intent);}
                if(deliteTurn&(position!=1)&(position!=0)){
                    String item =fruits.get(position);
                    shp_editor.remove(item).apply();
